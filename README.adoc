...
architecture-beta
    ...
    service alert_manager(server)[Alertmanager] in api
    service mailcatcher(server)[Mailcatcher] in api
    service adminer(server)[Adminer] in api
...
architecture-beta
    ...
    service db(database)[MSSQL DB] in api
